...
- **Piece set:** 7 shapes remain implemented. All five visible modes use the clearer I4/L4/T4/O4/J4 subset and a seedable 5-bag randomizer.
...
- **Modes:** Classic (the former Chill rules), Daily Challenge, fixed-orientation 2 Player, 2 Player Mirror, and 2 Player Puzzle are visible. Legacy Classic, Arcade, Sprint, Ultra, and Zen remain implemented but hidden after player feedback showed little use.
...
- **Quality bar:** 179 automated test cases in the suite, `flutter analyze` clean, CI on every PR (format, analyze, test, web build), deployed to GitHub Pages.
...
| **Daily Challenge** *(visible)* | Retention, repeatable daily puzzle | 8×16, starts with a deterministic terrain formation 2–7 rows high | Classic's 5 shapes, same formation and order for every player that day | Capped Classic curve | Reduce the locked stack to at most one occupied row; successful solves add up to 5,000 time-based points; unlimited retries, best result retained, streak advances once per day |
| **2 Player** *(visible)* | Online cooperative play through a shared six-character room code | One host-authoritative 8×20 board with two simultaneous falling pieces and a local-only landing ghost per peer | Classic's 5 shapes in independent seeded bags; host is always red/bottom-left and guest blue/top-right; each owns one starting cavity fill, and the player completing a line alone earns its recharge | Fixed 650 ms shared gravity | Normal shared top-out; level-scaled line/Fusion scoring, team combos, back-to-back hard-clear bonuses, and drop/lock points feed one shared score retained locally and on the friendly leaderboard; only the host can start a synchronized rematch for both peers |
...
| **2 Player Puzzle** *(visible)* | Daily-style cooperative puzzle through the same room-code flow | Host-authoritative 8×16 board seeded with a bottom-aligned 2–7-row terrain formation; local-only ghost per peer | Same five-shape bags and permanent red/blue ownership; both peers may Mirror and each begins with two personal cavity fills | Fixed 650 ms shared gravity | Win when the locked formation reaches at most one occupied row; successful solves add up to 5,000 host-timed points; top-out remains the loss condition; host-controlled rematches and an independent best/leaderboard keep puzzle scores separate |

Mode select intentionally presents **Classic**, **Daily Challenge**, **2 Player**, **2 Player Mirror**, and **2 Player Puzzle**. Timed and other legacy categories plus VIP remain hidden; Leaderboards and Login are available from the toolbar. Daily and 2 Player Puzzle are the visible win-condition modes. Fixed 2 Player keeps Mirror out of its accepted input protocol so complementary default orientations remain the cooperative constraint. Mirror and Puzzle accept Mirror for either peer while preserving permanent red/blue ownership. Fixed and Mirror give players separate one-charge fill inventories; Puzzle starts each player with two. Every cleared line recharges only the player whose action completed it. The host includes a round generation plus versioned effect events in authoritative peer snapshots, so both screens agree on rematches, clear/fusion/combo/back-to-back presentation, and puzzle victory while each peer still sees only its own landing ghost.

Both puzzle modes use the same completion incentive: `max(0, 5000 - 10 × whole active seconds)`, awarded only on a win. Daily's clock starts with the GO frame and excludes paused time. In 2 Player Puzzle, the host calculates the award once and includes it in the authoritative final score snapshot, preventing peers from disagreeing because of clock skew or network delay. During play, the HUD shows the currently available bonus; the completion presentation breaks it out from the total score.

Co-op presentation is score-first: the persistent responsive HUD gives the shared score the strongest type weight while keeping connection, best, level, current objective, combo/back-to-back, and red/blue clear contributions visible. The result overlay repeats the large team score and contribution split. Host-authoritative particles use red/blue ownership colors with a white readability core, hold before fading, and paint above board-wide flashes; impact rings inherit the acting player's color. A bounded live-particle pool prevents simultaneous locks or multi-line clears from creating unbounded frame work. Each action produces at most one high-contrast award card: its title prioritizes Triangle, combo, Fusion, clear, or cavity-fill importance while the subtitle preserves total, clear, Fusion, combo, and back-to-back point breakdowns. Cards hold for 1.85–2.4 seconds, and two effects arriving in one snapshot get a 260 ms visual-only stagger.
...
### 6.6 Leaderboards
Classic, Daily Challenge, fixed 2 Player, 2 Player Mirror, and 2 Player Puzzle shared-team scores have separate lightweight Firestore leaderboards. Each entry displays the player's chosen public, non-unique name rather than an email address or raw UID. The current no-Functions design accepts owner-written scores constrained by security rules and is suitable for friendly rankings, not cheat-resistant competition. Trusted replay validation remains a future requirement before prizes or high-stakes competitive claims; see `TECHNICAL_ARCHITECTURE.md` §4.
