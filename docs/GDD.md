...
- **Piece set:** 7 shapes remain implemented. The two visible modes use the clearer I4/L4/T4/O4/J4 subset and a seedable 5-bag randomizer.
...
- **Modes:** Classic (the former Chill rules), Daily Challenge, and shared-board 2 Player are visible. Legacy Classic, Arcade, Sprint, Ultra, and Zen remain implemented but hidden after player feedback showed little use.
...
- **Meta systems:** Stats & Achievements (29 achievements), 3 cosmetic themes, a 6-page reopenable tutorial, replay recording, Firebase Analytics, anonymous or optional email/password web accounts, authenticated Classic/Daily/2 Player leaderboards, and anonymous multiplayer signaling. Cloud backup, RevenueCat/VIP, Crashlytics, and native Firebase remain disabled (§10, `TECHNICAL_ARCHITECTURE.md`).
...
| **2 Player** *(visible)* | Online cooperative play through a shared six-character room code | One host-authoritative 8×20 board with two simultaneous falling pieces and a local-only landing ghost per peer | Classic's 5 shapes in independent seeded bags; host is always red/bottom-left and guest blue/top-right; each owns one starting cavity fill, and the player completing a line alone earns its recharge | Fixed 650 ms shared gravity | Normal shared top-out; shared team score is retained locally and on the friendly leaderboard; either peer can request a restart |

Mode select intentionally presents **Classic**, **Daily Challenge**, and **2 Player**. Timed and other legacy categories plus VIP remain hidden; Leaderboards and Login are available from the toolbar. Daily has a dedicated calendar card icon and remains the only visible mode with a win condition. In 2 Player, Mirror is removed rather than merely hidden: ownership of complementary triangle orientations is the cooperative constraint. Both players can fill cavities from separate one-charge inventories; each cleared line recharges only the player whose action completed it.
