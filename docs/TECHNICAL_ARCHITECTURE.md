...
3. **Optional Login (implemented):** the toolbar and lobby open one Login screen with a public player name, email, password, Log in, Create account, and Forgot password actions. Anonymous play remains the default and has the same leaderboard eligibility. Legacy/unnamed identities render a stable `Player ABC123` fallback.
4. **Create account:** `EmailAuthProvider.credential` is linked to the current anonymous user. Firebase therefore keeps the same UID and existing leaderboard entries. The normalized 3–20 character public name is stored in Firebase Auth's `displayName`; a verification email is requested after creation, but verification is not currently required for play.
5. **Returning devices:** `signInWithEmailAndPassword` restores that Firebase UID, chosen display name, and leaderboard identity. Local scores and settings remain device-local; logging into a different existing identity does not merge leaderboard documents or local saves.
...
3. `fetchTop`/`fetchTopMultiplayer` request at most 10 documents and cache each Classic/Daily/fixed-2-Player/Mirror-2-Player/Puzzle-2-Player board for the current app session. Only a successful submission or explicit Refresh causes another query.
4. Security rules allow an authenticated player to create or increase only their own small `{name, score, level, updatedAt}` document. They validate the path, fields, types, ranges, allowed name characters, server timestamp, and increasing-score direction. They also permit a name-only migration/update with score and level unchanged.
5. A changed Auth display name checks only the player's existing Classic, all three 2 Player variants, and current Daily documents. It writes only entries whose name changed, so a player with no scores causes zero leaderboard writes and historical Daily boards are not scanned.
...
`MultiplayerSessionService` uses Firestore only as a short-lived signaling rendezvous. A host creates a random six-character document containing the selected `fixed`, `mirror`, or `puzzle` variant and stores an SDP offer after ICE gathering, then shares the code out of band. The guest atomically claims that room, reads its variant, and stores its gathered answer. Candidates are embedded in those two descriptions rather than written as separate documents. A successful setup therefore still uses four room-document writes, and signaling listeners are cancelled once WebRTC opens the ordered data channel. No gameplay state is sent through Firestore.

The host runs the authoritative `CoopGameEngine`. The guest sends only actions; the host applies both players' ordered inputs and broadcasts compact, versioned snapshots. The original `fixed` variant rejects Mirror inputs. `mirror` accepts per-player Mirror input. `puzzle` also accepts Mirror, uses a seeded 16×8 board generated by the same deterministic 2–7-row terrain builder as Daily, gives each player two initial fills, and ends in victory when at most one occupied row remains. All variants preserve permanent red/blue ownership after a flip. Snapshot cell masks encode color and geometric half separately, preventing divergent line-clear, scoring, bonus, and simultaneous-lock resolution. Shared scoring uses level-scaled line and Fusion points, team combos, back-to-back bonuses, lock/drop points, and separate contribution totals. On a Puzzle victory, the host awards `max(0, 5000 - 10 × whole round seconds)` exactly once, adds it to the shared score, and serializes `puzzleSpeedBonus` in the final snapshot; the guest never calculates an independent result. Snapshots carry a monotonically increasing `roundId` before the per-round `revision`, the variant, puzzle victory state and speed bonus, both active mirrored states, combo/back-to-back state, both cavity-fill counts, and a bounded versioned effect history. Comparing `roundId` first fixes rematches because a guest accepts the new host engine even though its revision restarts near zero. Only the host can start a rematch; the guest result panel waits for the host, whose next snapshot starts both screens. Each peer derives and renders only its own landing ghost. At round end, each peer retains the shared score as its own local best for that variant, and contacts only the corresponding fixed, Mirror, or Puzzle leaderboard on a new best. The bonus uses the existing final-score submission path and adds no database write. No gameplay or rematch traffic touches Firestore. STUN is configured for development; production still requires TURN for restrictive NAT/firewall pairs.

`AudioService` remains the single owner of persisted master mute and independent music/SFX volumes. Gameplay HUDs observe it through `QuickMuteButton`; mode select exposes `MusicVolumeSlider`, while Settings retains both sliders. Music now defaults to 25%, and slider notifications happen before platform audio calls so the percentage responds immediately even when a web codec or test channel is slow. Serialized track requests prevent rapid route changes from racing: menu/lobby waiting loops `tmusic.mp3`, every gameplay mode loops `zen_classic_arcade_music.mp3`, and route disposal restores the menu loop.

Effect events stay on the same authoritative snapshot path, but rendering is local and creates no extra network traffic. Each peer turns those events into red/blue or Fusion particle bursts, centered hard-drop rings, line flashes, and consolidated score callouts. If a snapshot contains both players' events, a local 260 ms playback queue separates their visual starts without delaying input or authoritative state. Particles use a co-op-only 1.65× lifespan with an initial opacity hold; callouts combine all award fields into one 1.85–2.4-second card instead of launching overlapping labels. The painter caps active particles at 240 and discards the oldest first, giving simultaneous locks and multi-line clears a fixed upper bound on paint work. A repaint boundary isolates the animated board from the surrounding score HUD and controls.
...
The reporting dimensions are `mode`, `feature`, `action`, `result`, `role`, `reason`, and multiplayer `variant`; the useful metrics include duration, score, lines, `speed_bonus`, connection wait, completed rounds, and aggregate co-op controls. `speed_bonus` is attached to both solo `game_over` and `multiplayer_round_ended`, using zero for non-puzzle runs and losses. Custom parameters must be registered as custom definitions in Google Analytics before they are available in Explorations and custom reports. The same schema is summarized in the README's Analytics reporting section.
...
leaderboards/chill/entries/{uid}: { name, score, level, updatedAt }   // public name; owner may only increase score or update name

leaderboards/multiplayer/entries/{uid}: { name, score, level, updatedAt }   // player's best shared-team result

leaderboards/multiplayerMirror/entries/{uid}: { name, score, level, updatedAt }   // mirror-enabled shared-team result

leaderboards/multiplayerPuzzle/entries/{uid}: { name, score, level, updatedAt }   // cooperative puzzle result

dailyChallenge/{date}/entries/{uid}: { name, score, level, updatedAt }   // same owner-write model; date is YYYYMMDD

multiplayerRooms/{code}: { hostUid, guestUid?, variant, status, offer, answer?, createdAt, updatedAt, expiresAt }
```
...
`PRIVACY.md` now discloses Analytics, anonymous and email/password Firebase accounts, public chosen names, authenticated leaderboard fields and direct writes, Firestore signaling, and direct WebRTC gameplay. `TERMS.md` covers accounts, player-name conduct, leaderboard fair play, acceptable use, and online availability. Both are bundled into Settings → Legal. They remain product drafts requiring qualified legal review before broad commercial release.
