...
3. `fetchTop`/`fetchTopMultiplayer` request at most 10 documents and cache each Classic/Daily/fixed-2-Player/Mirror-2-Player board for the current app session. Only a successful submission or explicit Refresh causes another query.
...
5. A changed Auth display name checks only the player's existing Classic, both 2 Player variants, and current Daily documents. It writes only entries whose name changed, so a player with no scores causes zero leaderboard writes and historical Daily boards are not scanned.
...
`MultiplayerSessionService` uses Firestore only as a short-lived signaling rendezvous. A host creates a random six-character document containing the selected `fixed` or `mirror` variant and stores an SDP offer after ICE gathering, then shares the code out of band. The guest atomically claims that room, reads its variant, and stores its gathered answer. Candidates are embedded in those two descriptions rather than written as separate documents. A successful setup therefore still uses four room-document writes, and signaling listeners are cancelled once WebRTC opens the ordered data channel. No gameplay state is sent through Firestore.

The host runs the authoritative `CoopGameEngine`. The guest sends only actions; the host applies both players' ordered inputs and broadcasts compact, versioned snapshots. The original `fixed` variant rejects Mirror inputs and permanently starts red bottom-left and blue top-right. The separate `mirror` variant accepts a per-player Mirror input, persists each player's most recent orientation across their next spawn, and always retains the player's red/blue color. Snapshot cell masks encode color and geometric half separately, so a mirrored red top-right or blue bottom-left triangle reconstructs identically on both peers. This prevents divergent line-clear, scoring, bonus, and simultaneous-lock resolution. Shared scoring uses level-scaled line points and Fusion points, a team combo bonus, back-to-back bonuses for consecutive hard clears, lock/drop points, and separate red/blue line-contribution totals. Snapshots carry the variant, both active pieces' mirrored states, current/best combo, back-to-back count, separate red/blue cavity-fill counts, and a bounded history of versioned effect events; peers use those events to render the same particles, row flashes, impact rings, screen shake, toasts, haptics, level celebration, danger glow, and new-best presentation without additional signaling writes. The short history prevents either player's effect from being lost when both pieces lock during one gravity tick. Each player begins with one fill, and a line adds one only to the player whose action completed it. Each peer also derives a landing ghost locally from the same snapshot and renders only its own prediction; no extra network or Firestore traffic is required. At top-out, both peers retain the same shared score as their own local best for that variant. A peer contacts the corresponding fixed or Mirror leaderboard only for a new local best, and the standard transaction writes only if it also beats that Firebase player's remote best. STUN is configured for development. A production TURN service is still required because direct paths cannot be established across every NAT/firewall combination.
